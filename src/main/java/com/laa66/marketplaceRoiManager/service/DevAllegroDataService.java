...
public class DevAllegroDataService implements AllegroDataService {
    ...
    public List<CategoryDto> getCategoriesTree() {
        File file = new File("C:\\Users\\maxsc\\Desktop\\categories.json");
        try {
            BufferedReader reader = new BufferedReader(new FileReader(file));
            ObjectMapper mapper = new ObjectMapper();
            CategoriesDto categoriesDto = mapper.readValue(reader, CategoriesDto.class);
            return categoriesDto.getCategories();
        } catch (IOException e) {
    ...
    @Override
    @Deprecated
    public ResponseCategories getCategoryChildren(String categoryId) {
